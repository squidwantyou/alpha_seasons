...
for d in dirs:
    ...
    os.system(f"mkdir -p game_log/{player_name}")
    ...
    count = 0 
    fail_count = 0
    for line in open(f"table_ids/{player_name}_table_id.list") :
        inid = line.strip()

        if os.path.isfile(f"game_log/{player_name}/{inid}.json"):
            continue
        else:
            pass
        ...
        with open(f"game_log/{player_name}/{inid}.json",'w') as ofp:
            ofp.write(r.text)

        # test if success
        statue = True
        if os.path.getsize(f"game_log/{player_name}/{inid}.json") < 200:
            os.system(f"rm game_log/{player_name}/{inid}.json")
            statue = False
