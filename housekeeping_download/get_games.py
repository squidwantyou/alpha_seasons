...
for d in dirs:
    ...
    tmplines = open(d).readlines()
    # 
    player_id = tmplines[0].strip()
    LIMIT = 2000
    if os.path.isdir( f"games/{player_name}" ):
        continue
    os.system( f"mkdir -p games/{player_name}" )
    ...
    while True:
        ...
        with open(f"games/{player_name}/{i}.json",'w') as ofp:
            ofp.write(r.text)
        print(i,turl)

        if os.path.getsize(f"games/{player_name}/{i}.json") < 400 or i == LIMIT:
            break
        i += 1
